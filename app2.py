...
MASTER_CSV = os.path.join(DESEQ_DIR, "DESeq2_master_all_experiments.csv")
...
def get_deseq2_pvalues(deseq2_df, gene, deseq2_key):
    """
    Returns dict: treatment_group → raw DESeq2 pvalue (vs CONTR).
    Only includes comparisons that exist in the master table.
    """
    if deseq2_df is None or gene not in deseq2_df.index:
        return {}
    row = deseq2_df.loc[gene]
    # Handle duplicate gene names → take first row
    if isinstance(row, pd.DataFrame):
        row = row.iloc[0]
    pvals = {}
    for col in deseq2_df.columns:
        if col.startswith(f"{deseq2_key}__") and col.endswith('_pvalue'):
            # col format: ExptKey__TreatGroup_vs_CONTR_pvalue
            middle = col[len(deseq2_key)+2:]           # TreatGroup_vs_CONTR_pvalue
            treat  = middle.replace('_vs_CONTR_pvalue', '')
            val    = row[col]
            if pd.notna(val):
...
# ── Build full figure ─────────────────────────────────────────────────────────
def build_figure(gene, parsed, deseq2_df):
    row1_data = parsed[:5]
    row2_data = parsed[5:]
    ...
    for col, (label, ordered, groups, deseq2_key) in enumerate(row1_data):
        ...
        pvals = get_deseq2_pvalues(deseq2_df, gene, deseq2_key)
...
with st.sidebar:
    ...
    if os.path.exists(MASTER_CSV):
        ...
        st.error("❌ DESeq2_master_all_experiments.csv not found")
    for label, _, key in FILES:
        p = os.path.join(DESEQ_DIR, f"DESeq2_{key}.csv")
        if os.path.exists(p):
            st.success(f"✅ {key}")
        else:
            st.error(f"❌ DESeq2_{key}.csv missing")

# ── Main UI ───────────────────────────────────────────────────────────────────
gene_list  = load_gene_list()
deseq2_df  = load_deseq2()
...
if plot_button and gene_input:
    with st.spinner(f"Plotting {gene_input} across all 10 experiments…"):
        ...
        if all(len(p[2]) == 0 for p in parsed):
            ...
            if deseq2_df is not None and gene_input in deseq2_df.index:
                row = deseq2_df.loc[gene_input]
                if isinstance(row, pd.DataFrame):
                    row = row.iloc[0]
                pval_cols = [c for c in deseq2_df.columns if c.endswith('_pvalue')]
                summary_rows = []
                for col in pval_cols:
                    ...
                    expt, comparison = col.split('__')
                    treat = comparison.replace('_vs_CONTR_pvalue','')
                    summary_rows.append({
                        'Experiment': expt,
            ...
            fig = build_figure(gene_input, parsed, deseq2_df)
